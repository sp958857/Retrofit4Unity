...
namespace CI.HttpClient
{
    ...
    {
        public HttpHandler(HttpWebRequest request)
        {
            _request = request;
        }
...
#else
        public void SetContentHeaders(IHttpContent content)
        {
...
#if NETFX_CORE
        public void HandleRequestWrite(IHttpContent content, Action<UploadStatusMessage> uploadStatusCallback, int blockSize)
        {
...
#if NETFX_CORE
        public void HandleStringResponseRead(Action<HttpResponseMessage<string>> responseCallback)
        {
            try
            {
...
#if NETFX_CORE
        public void HandleByteArrayResponseRead(Action<HttpResponseMessage<byte[]>> responseCallback, HttpCompletionOption completionOption, int blockSize)
        {
            try
            {
                _response = (HttpWebResponse)_request.GetResponseAsync().Result;
        ...
        public void RaiseResponseCallback<T>(Action<HttpResponseMessage<T>> responseCallback, T data, long contentReadThisRound, long totalContentRead)
        {
        ...
        public void RaiseErrorResponse<T>(Action<HttpResponseMessage<T>> action, Exception exception)
        {
            if (action != null)
            {
